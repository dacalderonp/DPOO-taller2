...
public class SandboxListas
{
    ...
    public void insertarEntero( int entero, int posicion )
    {
        ...
        // Si la posición es mayor que el tamaño de la lista, inserta en la última posición
    ...
    public void reiniciarArregloEnteros( double[] valores )
    {
        ...
        // Trunca cada valor y lo agrega a la nueva lista
        for (double valor : valores) {
            int entero = (int) valor;  // Trunca el valor a un entero
            listaEnteros.add(entero);
        }
    }
    ...
    public void reiniciarArregloCadenas( List<Object> objetos )
    {
        ...
        // Convierte cada objeto a cadena utilizando toString y lo agrega a la nueva lista
        for (Object objeto : objetos) {
            String cadena = objeto.toString();  // Convierte el objeto a cadena
            listaCadenas.add(cadena);
        }
    ...
    public void volverPositivos( )
    {
        for (int i = 0; i < listaEnteros.size(); i++) {
            ...
            listaEnteros.set(i, Math.abs(valor));  // Utiliza Math.abs para obtener el valor absoluto
        }
    }
        ...
        for (int i = 0; i < listaEnteros.size() - 1; i++) {
            for (int j = i + 1; j < listaEnteros.size(); j++) {
                if (listaEnteros.get(i) < listaEnteros.get(j)) {
                    // Intercambia los elementos si el de la posición i es menor que el de la posición j
    ...
    public int contarEnterosRepetidos( )
    {
        ...
        for (int i = 0; i < listaEnteros.size() - 1; i++) {
            ...
            if (valorActual != Integer.MIN_VALUE) {  // Marca el valor como procesado
                int repeticiones = 1;

                for (int j = i + 1; j < listaEnteros.size(); j++) {
                    int siguienteValor = listaEnteros.get(j);
                    if (siguienteValor == valorActual) {
                        repeticiones++;
                        listaEnteros.set(j, Integer.MIN_VALUE);  // Marca el valor como procesado
                    }
                }
    ...
    public boolean compararArregloEnteros( int[] otroArreglo )
    {
        if (listaEnteros.size() != otroArreglo.length) {
            return false;  // si no pueden ser iguales
        }

        for (int i = 0; i < listaEnteros.size(); i++) {
            if (listaEnteros.get(i) != otroArreglo[i]) {
                return false;  // si no coincide, no son iguales
            }
        }
    ...
    public void generarEnteros( int cantidad, int minimo, int maximo )
    {
        ...
        for (int i = 0; i < cantidad; i++) {
            ...
            int nuevoEntero = (int) (Math.random() * rango) + minimo;
